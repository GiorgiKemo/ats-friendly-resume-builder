...
const TestimonialCard = ({ quote, author, position, company }) => (
  <motion.div
    className="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-md dark:shadow-slate-700/30 transition-shadow duration-200 ease-out hover:shadow-lg will-change-transform"
    whileHover={{ y: -4 }}
  ...
  >
    ...
    <p className="italic text-gray-600 dark:text-slate-400 mb-4">{quote}</p>
    <div>
      <p className="font-semibold">{author}</p>
      <p className="text-sm text-gray-500 dark:text-slate-500">{position}, {company}</p>
    </div>
  </motion.div>
);
...
const TestimonialsSection = () => {
  ...
  return (
    <div className="py-16 bg-blue-50 dark:bg-blue-900/20">
