 ...
 * Contains country codes, names, and dial codes
 */
 ...
 * @param {string} dialCode - The dial code to search for
...
export const getCountryByDialCode = (dialCode) => {
  ...
  const cleanDialCode = dialCode.startsWith('+') ? dialCode.substring(1) : dialCode;
 ...
 * @param {string} dialCode - The dial code to prepend
 * @returns {string} - The formatted phone number
 */
export const formatPhoneNumberWithCountryCode = (phoneNumber, dialCode) => {
  if (!phoneNumber) return '';
  
  // Remove the dial code from the phone number if it already exists
  let cleanPhoneNumber = phoneNumber;
  if (dialCode && phoneNumber.startsWith(dialCode)) {
    cleanPhoneNumber = phoneNumber.substring(dialCode.length);
  }
  
  // Remove any non-digit characters from the phone number
  cleanPhoneNumber = cleanPhoneNumber.replace(/\D/g, '');
  
  return dialCode ? `${dialCode} ${cleanPhoneNumber}` : cleanPhoneNumber;
};
