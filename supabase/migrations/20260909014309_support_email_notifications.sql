...
create or replace function public.support_enqueue_agent_email()
returns trigger
language plpgsql
security definer
...
begin
  if new.sender_type = 'agent' then
    ...
    values (new.conversation_id, new.id, 'email')
...
drop trigger if exists support_agent_message_email on public.support_messages;
create trigger support_agent_message_email
after insert on public.support_messages
for each row execute function public.support_enqueue_agent_email();
...
revoke all on function public.support_enqueue_agent_email() from public, anon, authenticated;
