...
11. **Support-session identity boundary — deployed; authenticated behavior gate
    ...
    current READY deployment `dpl_8oUwdhhQDxpLtojKG2fRzUN1JpNe`, built from
    `166cd4c`; no real authenticated support message was submitted.
...
13. **Password-recovery normalization — deployed; provider behavior gate remains
    ...
    READY deployment `dpl_8oUwdhhQDxpLtojKG2fRzUN1JpNe` from `166cd4c`.
...
- The global pointer-focus hardening used for Step 9 is present in the current
  live CSS (deployment `dpl_8oUwdhhQDxpLtojKG2fRzUN1JpNe`, built from
  `166cd4c`) and is also covered by the pushed regression test. The same bundle
  ...
  The live bundle is deployment `dpl_8oUwdhhQDxpLtojKG2fRzUN1JpNe`, built from
  `166cd4c`.
...
- Newsletter subscription feedback is deployed in `f37b92e` through the current
  `166cd4c` production build. A fetch of the live homepage bundle found the
