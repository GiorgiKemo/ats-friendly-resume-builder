...
- Dependency parity: non-major versions within the existing semver ranges were refreshed with `npm update`; the refreshed lockfile passes `npm ls --depth=0`, 1,190 tests, lint, production build, Supabase function check, route smoke, and `npm audit --audit-level=high` with zero findings. Major framework upgrades remain intentionally unmerged pending a dedicated compatibility migration.
