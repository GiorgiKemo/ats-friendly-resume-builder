...
- Production remains at 80 migrations. The migration and function change are not deployed; this adds local E02/A01 evidence but does not establish production session revocation, Data API/Storage behavior, or staging readiness. Details: [`20260924-admin-session-guard.md`](20260924-admin-session-guard.md).
...
| --- | --- | --- | --- | --- |
| E00 | in_progress | 82 local / 80 production migrations; read-only Management API confirms the active ResumeATS project and production migration head `20260923204601` | Linked Supabase audit (59/59 public tables RLS-enabled, 29 policies, 133 public functions, one active owner), 29 local / 31 deployed functions with none missing/inactive, default ACL verification, GA4 review, and two local session-guard migrations; see `20260924-production-capability-audit.md`, `20260924-admin-session-guard.md`, and `20260924-direct-support-session-guard.md` | Neither session-guard migration is applied in production. Worker secrets, scheduler, email/scanner configuration, backup/restore and provider ownership remain open; authenticated production role-behavior tests remain incomplete. |
...
| E02 | in_progress | Local admin API and selected direct admin/support RPCs validate signed session identity; service-role attachment finalize/download validates exact user/session; production functions remain unchanged | Local active, revoked, missing-claim, expired-session, attachment-preparation, and database-error tests plus synthetic owner/customer capability checks; see `20260924-admin-session-guard.md` and `20260924-direct-support-session-guard.md` | Production migration-first/function-second rollout and authenticated owner/revoked-session tests are outstanding. |
| E03 | in_progress | 82 local / 80 production migrations; current local schema lint has no errors; default API-object grants hardened in production | 82-migration isolated replay; default-ACL and public SECURITY DEFINER role/search-path assertions; service-only and direct session predicates; attachment preparation denial for expired sessions; synthetic owner/customer support isolation and internal-note secrecy tests. Production grant summary is in `20260924-production-capability-audit.md`. | Session-guard migrations are not deployed; production authenticated user/owner/support behavior and direct API/RPC/Storage matrix still need verification. |
...
| E07 | in_progress | Consent-gated GA4 funnel; `sign_up` is deployed and marked as a GA4 key event without a default monetary value | Passing unit/auth tests, production Vercel success, live HTTP/asset verification, and ResumeATS key-event UI confirmation in `20260924-release-verification.md` | Processed-event receipt for a real consented signup, authenticated conversion journey, and server-side reporting credentials remain open. |
...
| E18 | blocked | 2026-09-23/24 local checks pass; signup instrumentation release verified; local 82-migration session guard tested; CI passed on diagnostic commit `44e1ab2` | 1,285 tests; lint, build, repo/function checks, 82-migration isolated replay, local schema lint, 17-route accessibility audit, 17 fixture journeys, 2026-09-23/24 Supabase-backed support browser flows/screenshots, 32-route smoke, prior Chromium/Firefox extension and authenticated production admin evidence, plus the live release check in `20260924-release-verification.md`; support session evidence in `20260924-direct-support-session-guard.md` | Actual supported-browser staging/provider/performance/accessibility evidence and authenticated production journeys are incomplete; both session guards are not deployed. |
...
| E20 | in_progress | Latest source `44e1ab2529af28458aee19cbd19b5f52137822e9`; the earlier runtime commit `484c672d0f5ebb942b7fd95c5696e9c32d4e7ced` has a `READY` Vercel deployment; the current direct-support guard is still local | Production domain alias resolved and returned HTTP 200 for the verified deployment. GitHub CI run `35928817384` passed. Prior live evidence includes signup-bundle and GA4 key-event confirmation, privacy/support/pricing browser flows, Search Console evidence, Supabase function deployment, applied policy migrations, and authenticated production admin/deep-link QA in `20260910-production-admin-qa.md` and `20260924-release-verification.md` | The current working-tree guard has not been pushed or deployed; neither session-guard migration is in production. Provider/scheduler/backup/restore/worker-credential/authenticated journey gates remain open. |
...
1. Complete the existing-object grant/inheritance review and production role/session behavior matrix across REST/RPC/Storage; the 2026-09-24 metadata snapshot is documented but is not a substitute for row-level tests.
