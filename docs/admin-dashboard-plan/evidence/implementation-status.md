...
| E03 | in_progress | 80 local / 80 linked production migrations; local schema lint clean; default API-object grants hardened in production | 80-migration isolated replay; full table/sequence/function default-ACL assertions locally and in production; 59/59 public tables RLS-enabled; migration `20260910035425_remove_legacy_public_users_select_policy.sql` and the new ACL migration are applied. Production grants/policy summary is in `20260924-production-capability-audit.md`. | Production authenticated user/owner/support behavior and direct API/RPC/Storage matrix still need isolated production-safe verification; RLS enablement alone does not prove row-level behavior. |
