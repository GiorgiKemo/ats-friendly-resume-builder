...
## Current production release — backend and website deployed (2026-09-24)

- Production now has 89/89 migrations with zero history drift. Thirteen coupled functions are active and retain their previous JWT modes; the exact version inventory and boundaries are in [`20260924-production-release-final.md`](20260924-production-release-final.md).
- Website commit `93b17f1` is recorded as pushed to `origin/main`; Vercel deployment `dpl_7dAqfsj1HcjWgfQwW9QUuxAMbjvL` reached `READY` on both canonical aliases. The post-promotion HTTP/asset audit at `2026-09-24T11:54:25.558Z` passed with zero failures; post-deploy CORS preflights pass for all five consent-aware AI endpoints.
- Current local checks: 1,340 unit tests, lint, build, repository hygiene, Edge Function type-check, and all 91 local PostgreSQL 17 migration replays pass. The current working tree has two local-only E08 cohort migrations beyond the 89-migration production release. The 18-step customer fixture flow, support/admin browser suites, 200% zoom, and responsive builder checks pass within their documented synthetic-data boundaries; the latest Chromium run verifies the Product retention D7/D30 cards render and reports no console/page errors or contrast findings.
- Owner/operations gates remain: 15 missing worker/config values, no scheduler jobs, no verified production owner TOTP factor, no full authenticated production role/session matrix, provider sandbox reconciliation, monitoring, and backup/restore drills. No production secrets or Auth settings were changed during this release.
...
| --- | --- | --- | --- | --- |
| E00 | in_progress | Production project `ACTIVE_HEALTHY`; 91 local / 89 production migrations, with two local-only E08 migrations; project-local PAT's documented read-only endpoint is `supabase_read_only_user` with transaction read-only enabled; latest account inventory found no ResumeATS staging project | Fresh production catalog inventory confirms 59/59 public tables RLS-enabled, 29 public policies, 134 public functions (106 `SECURITY DEFINER`, all search paths pinned), 8/8 Storage tables RLS-enabled, and 31 active deployed functions; two deployed function slugs have no local source directory; isolated PostgreSQL and loopback Storage HTTP fixtures cover synthetic resume-folder and support attachment behavior. Read-only refresh: `20260924-e00-access-refresh.md`; detailed catalog evidence: `20260924-production-readonly-grants-inventory.md` | Production REST/RPC/Storage HTTP role/session behavior across the full customer/guest/support/billing/admin/owner/revoked/stale matrix remains untested. The separate `supabase_admin` defaults do not apply to current `postgres`-owned objects; preserve this boundary and recheck migration actor/owners on staging. Worker secrets, scheduler, email/scanner configuration, backup/restore, and provider ownership remain outstanding. |
...
| E08 | in_progress | First-party analytics/admin reporting backend and owner-only AAL2 QA exclusion API are deployed; timezone-bounded reporting, export event wiring, 7-day activation, and exact-calendar-day D7/D30 retention cohorts are implemented locally; current UI delta is not deployed | 1,340 unit tests; backend snapshot/API boundary tests; 91-migration disposable PostgreSQL replay proves 4/10 activation, 3/10 D7 and 3/10 D30 observed fixtures, exact timezone day boundaries, deduplication, QA exclusions and maturing-account states; Chromium browser flow asserts both retention cards render; saved GA4 `ResumeATS Growth & Conversion` report `15836594687` | All activation/retention final rates remain withheld because event coverage is incomplete or consent-limited. Daily aggregates/rebuild, cached GA reporting, server-side API access, real consented event receipts, mature first-party data and authenticated production drill-down remain open. |
...
| E18 | blocked | Local code checks pass; production release has 89/89 migrations and 13 coupled Edge Functions active; latest E08 local migrations/UI are not deployed | 1,340 tests; lint/build/function/repo checks; 91-migration replay and targeted paid/activation/retention/QA/support fixtures; schema lint; public/auth accessibility DOM audit; 18-step fixture browser flow; 32-route smoke test; Chromium/Firefox/WebKit support/admin AAL2 browser flows; responsive and 200% zoom checks; contrast audits; latest Chromium retention-card render assertions. | Supported-browser staging/provider/performance/accessibility evidence, authenticated production journeys, provider sandbox checks, and backup/restore evidence remain incomplete. |
...
| E20 | in_progress | Checkout `main` at `3fc4a77` matches `origin/main`; production remains at the 89-migration release; current working-tree E08 code/migrations are not pushed or deployed | Production release evidence records the prior Vercel `READY` deployment; current local evidence is 1,340 tests, lint/build/function/repo checks, and 91-migration replay. No production migration, function, secret, or Auth setting was changed in this work cycle. | The current E08 delta still needs an authorized GitHub/Vercel release and independent live verification; worker credentials, scheduler/alerts, backup/restore, provider sandbox checks, authenticated production journeys, and separate staging availability also remain open. |
