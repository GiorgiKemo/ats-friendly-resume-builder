...
| --- | --- | --- | --- | --- |
| E00 | in_progress | 80 local / 80 linked production migrations; fresh read-only capability and advisor checks | Linked Supabase audit (59/59 public tables RLS-enabled, 29 policies, 133 public functions, one active owner), 29 local / 31 deployed functions with none missing/inactive, default ACL verification, and GA4 review; details in `20260924-production-capability-audit.md` | Worker secrets, scheduler, email/scanner configuration, backup/restore and provider ownership remain open; the audit reads secret names only. Authenticated production role-behavior tests remain incomplete. |
...
| E02 | in_progress | Current admin migrations/functions | Local authorization, AAL2, idempotency and audit tests; isolated ordinary-customer denials for admin/support/knowledge RPCs | Production authenticated role/session matrix has not been exercised. |
| E03 | in_progress | 80 local / 80 linked production migrations; local schema lint clean; default API-object grants hardened in production | 80-migration isolated replay; default-ACL and public SECURITY DEFINER role/search-path assertions; synthetic owner/customer support isolation and internal-note secrecy tests; 59/59 public tables RLS-enabled. Production grants/policy summary is in `20260924-production-capability-audit.md`. | Production authenticated user/owner/support behavior and direct API/RPC/Storage matrix still need production-safe verification; RLS enablement alone does not prove row-level behavior. |
