...
- The latest read-only capability snapshot observed 79 linked production migration versions before this new local migration. Linked production database query and migration dry-run returned HTTP 403 for insufficient Supabase privileges. No production schema change was made; production remains at 79 until an authorized migration can be applied and parity rechecked.
...
| --- | --- | --- | --- | --- |
| E00 | in_progress | 80 local / 79 linked production migration versions; latest read-only capability snapshot predates local ACL migration | Local checkout, linked Supabase capability audit, production HTTP audit, privacy-safe database metadata (59 public tables / 59 RLS-enabled; one active owner member), and GA4 UI review in `20260923-local-verification.md`; latest ACL follow-up in `20260924-default-public-grants.md` | Provider ownership, scheduler, backups, worker credentials, email configuration, and authorized production DB access remain open; scheduler metadata reports no available `pg_cron`/`pg_net`. |
...
| E03 | in_progress | 80 local / 79 linked production migration versions; local schema lint passes with no errors; legacy public profile policy cleanup applied | 80-migration isolated replay, full future-object ACL assertions, local migration and transactional ACL probe, database-metadata audit, and anonymous REST probe; migration `20260910035425_remove_legacy_public_users_select_policy.sql` is applied and the owner-read policy is `authenticated`-only; ACL details in `20260924-default-public-grants.md` | Production migration dry-run/query returned HTTP 403; the new default-privilege migration is not applied to production. A complete production grants/membership and row-level behavior evidence package remains unavailable. |
...
| E20 | in_progress | Signup analytics release `6515a63317629b4c2c2acb39eb6e3f91ebff717f`; prior promoted `dpl_CfsUrKpwvXtkNQvRdyaGPR8qbgqK` (`cf38adb`), `dpl_AXjy2btB1ndRieNtUyG7cwrdXYQo`, `dpl_ARpryNMhtDJneUzsnxv6bYDh6j6s`, `dpl_F8M3Q2c4oHLibRQxyAzYyUL9XFpG`, `dpl_9cQow2P8ytqafYdFvWd5N3M2vY7J` (`8d66479`), `dpl_6rwe8XUB1chtRfZpcDu8Tm9e879c` (`0a1e6bc`) and `dpl_mf8N6LwBcoPervofFk7CYiuPsmsd` (`dbd199c`); failed build `dpl_HfqvM7y7feQN4vZhH6GpEYCe2ejV`; evidence source `34efaff`; packaging source `295b528`/`8d0f805`; billing source `943ad68`/`c08bafd`; policy cleanup source `d2a0d14` | GitHub push, Vercel status, fresh live HTTP audit and signup-bundle verification, GA4 key-event confirmation, live privacy-request browser flow, live FAQ/support browser flow, live pricing selector flow, live GA client check, Search Console evidence, live CSP tightening, route-focus and admin export/billing fixes in promoted production, targeted Supabase function deployment, applied policy migration, and authenticated production admin/deep-link QA in `evidence/20260910-production-admin-qa.md` and `evidence/20260924-release-verification.md` | The full completion gate is not met while any required integration remains unverified, inaccessible or intentionally disabled; provider/scheduler/backup/restore/worker-credential/authenticated journey gates remain open. |
