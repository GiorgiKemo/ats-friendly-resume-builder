...
| E18 | blocked | 2026-09-23/24 local checks pass; signup instrumentation release verified; local 81-migration session guard tested; latest hosted CI has one replay assertion failure | 1,282 tests; lint, build, repo/function checks, three 81-migration isolated replays, 17-route accessibility audit, 17 fixture journeys, 2026-09-23/24 Supabase-backed support browser flows/screenshots, 32-route smoke, prior Chromium/Firefox extension and authenticated production admin evidence, plus the live release check in `20260924-release-verification.md` | CI diagnostic follow-up is in progress; actual supported-browser staging/provider/performance/accessibility evidence and authenticated production journeys are incomplete; session guard is not deployed. |
...
| E20 | in_progress | Latest source `484c672d0f5ebb942b7fd95c5696e9c32d4e7ced`; Vercel deployment `dpl_3VZ6JBjPEd4s5o2LRmL34MqztbHQ` for that commit is `READY`; earlier release chain remains in Git history | Production domain alias resolves and returned HTTP 200. GitHub CodeQL passed; latest `lint-and-build` failed at the 16-way resume-save conflict-count assertion after the new 81-migration session guard tests passed. Prior live evidence includes signup-bundle and GA4 key-event confirmation, privacy/support/pricing browser flows, Search Console evidence, Supabase function deployment, applied policy migrations, and authenticated production admin/deep-link QA in `20260910-production-admin-qa.md` and `20260924-release-verification.md` | CI follow-up remains open; the session-guard migration/function are not deployed; provider/scheduler/backup/restore/worker-credential/authenticated journey gates remain open. |
