...
## Current production release — analytics backend deployed; website follow-up pending (2026-09-24)
...
- Website commit `dea56ff` was pushed to `origin/main`; Vercel deployment `dpl_CjZeU3W51Umz9TCMvGwqbVbx2TwU` reached `READY` on both canonical aliases. The HTTP/asset audit at `2026-09-24T14:10:08.067Z` passed with zero failures after the backend update. GitHub CI rerun `36006074497` passed for that website commit.
- Current local checks: 1,347 unit tests, lint, build, repository hygiene, Edge Function type-check, and all 93 local PostgreSQL 17 migration replays pass. The 18-step customer fixture flow, support/admin browser suites, 200% zoom, and responsive builder checks pass within their documented synthetic-data boundaries; the latest Chromium run verifies AAL1 aggregate-rebuild denial, AAL2 owner rebuild success, the Product retention D7/D30 cards, and the local subscription run-rate panel, with no console/page errors, contrast findings, or overflow. The run-rate panel and guard that disables rebuild until AAL2 are awaiting website deployment.
...
| E08 | in_progress | First-party analytics/admin reporting backend, owner-only AAL2 QA exclusion API, daily aggregate migration 92, subscription run-rate migration 93 and `admin-api` v24 are deployed; daily aggregate panel is live, while the currency-separated preview panel is in the pending website follow-up | 1,347 unit tests; 93-migration disposable PostgreSQL replay proves 4/10 activation, 3/10 D7 and 3/10 D30 observed fixtures, timezone boundaries, deduplication, QA exclusions, maturity, service-only aggregate access, zero-fill/generation invalidation, and incomplete currency-separated run-rate normalization; Chromium browser flow verifies AAL1 server denial, AAL2 owner rebuild and local run-rate panel rendering; saved GA4 `ResumeATS Growth & Conversion` report `15836594687` | The run-rate panel and AAL2-disabled rebuild control are locally verified and pending website deployment. All activation/retention final rates remain withheld because event coverage is incomplete or consent-limited. Cached GA reporting, server-side API access, real consented event receipts, mature first-party data and authenticated production drill-down remain open. |
...
| E18 | blocked | Local code checks pass; production has 93/93 migrations and 13 coupled Edge Functions active, including `admin-api` v24; the run-rate panel and MFA-state UX follow-up are local | 1,347 tests; lint/build/function/repo checks; 93-migration replay and targeted paid/activation/retention/QA/support/aggregate/run-rate fixtures; schema lint; public/auth accessibility DOM audit; 18-step fixture browser flow; 32-route smoke test; Chromium/Firefox/WebKit support/admin AAL2 browser flows; responsive and 200% zoom checks; contrast audits; latest Chromium AAL1-denial/AAL2-rebuild and retention-card assertions. | Supported-browser staging/provider/performance/accessibility evidence, authenticated production journeys, provider sandbox checks, and backup/restore evidence remain incomplete. |
...
| E20 | in_progress | `origin/main` is at `dea56ff`; Vercel `dpl_CjZeU3W51Umz9TCMvGwqbVbx2TwU` is `READY` on canonical aliases; the run-rate panel and AAL2 control-state usability follow-up are local and pending release | The deployed release has migrations 92–93 and `admin-api` v24; CI rerun `36006074497` passed for `dea56ff`, and live HTTP/asset verification returned zero failures. Current local website adds the currency-separated run-rate panel and MFA-state UX guard with AAL1/AAL2 browser assertions. No production Auth setting, Vault secret, or unrelated Function secret was changed. | Deploy and independently verify the local website follow-up; worker credentials, scheduler/alerts, backup/restore, provider sandbox checks, authenticated production journeys, and separate staging availability remain open. |
