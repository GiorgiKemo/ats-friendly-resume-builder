...
- Focused backend tests passed 17/17; the isolated PostgreSQL 17 migration replay passed all 81 local migrations and tested role grants, active/matching, expired, missing, and mismatched sessions. The latest `npm test` passed 1,282/1,282; lint, repo hygiene, Supabase function checks, and production build passed.
...
| --- | --- | --- | --- | --- |
| E00 | in_progress | 81 local / 80 linked production migrations after the new local session-guard migration; fresh read-only capability and advisor checks | Linked Supabase audit (59/59 public tables RLS-enabled, 29 policies, 133 public functions, one active owner), 29 local / 31 deployed functions with none missing/inactive, default ACL verification, GA4 review, and read-only Auth-session schema check; see `20260924-production-capability-audit.md` and `20260924-admin-session-guard.md` | New migration is not applied in production. Worker secrets, scheduler, email/scanner configuration, backup/restore and provider ownership remain open; authenticated production role-behavior tests remain incomplete. |
...
| E02 | in_progress | Local admin API now validates signed session identity against a service-only active-session RPC; production function remains unchanged | Local active, revoked, missing-claim, and database-error tests plus synthetic owner/customer capability checks; see `20260924-admin-session-guard.md` | Production migration-first/function-second rollout and authenticated owner/revoked-session tests are outstanding. |
| E03 | in_progress | 81 local / 80 linked production migrations; prior local schema lint predates the 81st migration; default API-object grants hardened in production | 81-migration isolated replay; default-ACL and public SECURITY DEFINER role/search-path assertions; service-only session RPC grant and session-state behavior; synthetic owner/customer support isolation and internal-note secrecy tests. Production grant summary is in `20260924-production-capability-audit.md`. | New migration is not deployed; production authenticated user/owner/support behavior and direct API/RPC/Storage matrix still need verification. |
...
| E18 | blocked | 2026-09-23/24 automated/local checks pass; signup instrumentation release verified; local 81-migration session guard tested | 1,282 tests; lint, build, repo/function checks, 81-migration isolated replay, 17-route accessibility audit, 17 fixture journeys, 2026-09-23/24 Supabase-backed support browser flows/screenshots, 32-route smoke, prior Chromium/Firefox extension and authenticated production admin evidence, plus the live release check in `20260924-release-verification.md` | Actual supported-browser staging/provider/performance/accessibility evidence and authenticated production journeys are incomplete; session guard is not deployed. |
...
| E20 | in_progress | Latest source `fba83dcbdbd9a8c79298674db64407b17e42edd0`; Vercel production deployment `dpl_3ppLvgmHWFrMxs2k6GXnkmoks19n` is `READY`; earlier signup release `6515a63317629b4c2c2acb39eb6e3f91ebff717f` and release chain retained in Git history | GitHub CI passed; production HTTP audit returned no failures. Prior live evidence includes signup-bundle and GA4 key-event confirmation, privacy/support/pricing browser flows, Search Console evidence, Supabase function deployment, applied policy migrations, and authenticated production admin/deep-link QA in `20260910-production-admin-qa.md` and `20260924-release-verification.md` | The full completion gate is not met while any required integration remains unverified, inaccessible or intentionally disabled; provider/scheduler/backup/restore/worker-credential/authenticated journey gates remain open. |
