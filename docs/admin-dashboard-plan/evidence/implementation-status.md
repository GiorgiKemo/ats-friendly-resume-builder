...
| E07 | in_progress | Consent-gated GA4 funnel; local email signup now emits PII-free `sign_up` only for a non-empty Supabase identity response | Existing live GA4 property/stream and processed-report evidence in `20260910-ga4-dashboard.md`; fresh 2026-09-23 report observation and passing unit/auth tests in `20260923-local-verification.md` | The code change is not yet released; GA4 `sign_up` key-event configuration, post-release processed-event receipt, authenticated conversion journeys, and server-side reporting credentials remain open. |
...
| E18 | blocked | 2026-09-23/24 automated/local checks pass | 1,281 tests; lint, build, repo/function checks, local SQL lint, 17-route accessibility audit, 17 fixture journeys, 2026-09-23/24 Supabase-backed support browser flows/screenshots, 32-route smoke, plus prior Chromium/Firefox extension and authenticated production admin evidence | Actual supported-browser staging/provider/performance/accessibility evidence and authenticated production journeys are incomplete. |
