...
- The owner/admin settings UI exposes the kill switch, bounded limits, budget fields, recent revision history, an audited AAL2 circuit-clear action, and a confirmed audited rollback to an older revision; provider/model identity remains environment/configuration-owned and secret values are never returned. Automatic circuit tuning and longer-range history/reporting remain separate work.
- The worker reads the saved response-token and conversation-reply limits before claiming work, applies the token cap to provider requests, and routes a conversation to the human queue at its reply cap without calling the provider. The database kill switch gates claims; missing or invalid runtime limits fail closed.
- The Admin API refuses both direct enablement and rollback to an enabled revision unless the `SUPPORT_AI_ENABLED` runtime flag is on; either path also requires provider/worker configuration. The UI keeps the enable control disabled until the runtime flag plus provider/worker configuration are present. This prevents the database trigger from moving new customer messages into AI mode while the worker is disabled.
- Daily/monthly USD entries are persisted as settings but are **not enforced spending caps**. Provider-reported cost units/currency are not verified, and there is no atomic reservation against an approved provider rate card yet. Do not use these fields to justify enabling the provider; the UI now says so.
