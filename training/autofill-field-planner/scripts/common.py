...
- Do not fill vague optional textareas such as "additional information", "anything else", "comments", or "notes" unless the profile has an explicit answer for that exact prompt.
...
def sanitize_planner_output(plan: dict, input_payload: dict) -> dict:
    ...
    for action in plan.get("actions", []):
        ...
        if _is_generic_optional_text_field(field) and not skip:
            ...
            if source != "explicit_profile" or not _matches_explicit_generic_answer(selected_text, explicit_generic_answers):
                notes.append(f"Skipped vague optional field {field_id!r}; no explicit profile answer was available.")
                value = ""
                option_text = ""
                confidence = "low"
                source = "human_review"
