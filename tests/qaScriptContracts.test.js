...
test('support browser QA exercises the admin AI and job status panels', () => {
  ...
  assert.match(supportQa, /const configuredSupportQaPort = process\.env\.SUPPORT_QA_PORT;/);
  assert.match(supportQa, /const candidates = configuredSupportQaPort \? \[configuredSupportQaPort\] : \['5176', '5175', '5174', '5173'\]/);
  assert.match(supportQa, /await navigateAdminTo\('\/admin\/users', \{ waitUntil: 'networkidle' \}\)/);
  assert.match(supportQa, /await navigateAdminTo\(`\/admin\/users\/\$\{ownerId\}`, \{ waitUntil: 'networkidle' \}\)/);
  ...
  assert.match(supportQa, /\.admin-nav'\)\.getByRole\('button', \{ name: 'Analytics'/);
  assert.match(supportQa, /\.admin-nav'\)\.getByRole\('button', \{ name: 'Support'/);
...
test('Vercel uploads only runtime inputs, not local QA artifacts or Edge sources', () => {
  ...
  for (const entry of ['docs', 'output', 'playwright-audit', 'playwright-artifacts-*', 'tmp-extension-*', 'supabase']) {
    if (entry === 'supabase') {
      ...
      assert.match(vercelIgnore, /^!supabase\/functions\/_shared$/m);
      assert.match(vercelIgnore, /^!supabase\/functions\/_shared\/\*\*$/m);
    } else {
