...
function setup({ user = null, loading = false } = {}) {
  ...
  const app = componentHarness('src/components/auth/ForgotPassword.jsx', {
    ...
    imports: {
      'react-router-dom': { Link: 'Link', Navigate: 'Navigate' },
      '../../services/supabase': { supabase: { auth: { resetPasswordForEmail: (...args) => { calls.push(args); return request.promise; } } } },
      '../ui/Input': { default: 'Input' }, '../ui/Button': { default: 'Button' }, '../ui/PageHero': { default: 'PageHero' },
...
test('authenticated users are sent back to the workspace instead of requesting a reset link', () => {
  const app = setup({ user: { id: 'user-1' } });
  const redirect = find(app.render(), (node) => node.type === 'Navigate');
  assert.equal(redirect.props.to, '/dashboard');
...
test('recovery form submits once and uses a non-enumerating, accessible success message', async () => {
  ...
  assert.ok(find(tree, (node) => node.type === 'PageHero' || node.type === 'h1'));
