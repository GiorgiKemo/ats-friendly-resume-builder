...
test('app redirects normalize router state and billing return paths to same-origin routes', () => {
  ...
  assert.match(main, /getSafeInternalPath\(hash\.slice\(1\) \|\| '\/', '\/'\)/);
  ...
  assert.match(stripeReturn, /getSafeInternalPath\(redirectPath, '\/dashboard'\)/);
  assert.doesNotMatch(stripeReturn, /const getSafeRedirectPath/);
...
test('auto-apply admin controls are audited, idempotent, and fail closed around outbound work', () => {
  ...
  const jobOperationsReader = adminApi.slice(adminApi.indexOf('const fetchAdminJobOperations ='), adminApi.indexOf('const safeEventCount ='));
  assert.match(jobOperationsReader, /\.select\('operation_id,job_id,action,status,created_at,updated_at'\)/);
  assert.match(jobOperationsReader, /operationId: lastAction\.operation_id/);
  assert.doesNotMatch(jobOperationsReader, /failure_reason|failureReason|lastAction\.(?:reason|result)/i);
  assert.doesNotMatch(jobOperationsReader, /\.select\('[^']*(?:,reason|,result)(?:,|')/i);
  assert.match(dashboard, /Operation ID: \{lastAction\.operationId/);
...
test('feedback insights and improvement backlog stay operator-scoped, sanitized, and idempotent', () => {
  ...
  assert.match(dashboard, /const AdminFeedbackPanel = \(\{ operators = \[\] \}\)/);
  assert.match(dashboard, /ownerUserId: changes\.ownerUserId !== undefined \? changes\.ownerUserId \|\| null : item\.ownerUserId \|\| null/);
  assert.match(dashboard, /Owner<select[\s\S]*?item\.ownerUserId \|\| ''[\s\S]*?operators\.map\(\(operator\)/);
  assert.match(dashboard, /AdminFeedbackPanel operators=\{adminMembers\.filter\(\(member\) => member\.is_active && member\.user_id\)\}/);
...
test('support AI is disabled by default and uses a durable guarded worker boundary', () => {
  ...
  const usageMigration = read('supabase/migrations/20260924174548_admin_support_ai_usage_telemetry.sql');
  const emailHealthMigration = read('supabase/migrations/20260924180906_admin_support_email_delivery_health.sql');
  ...
  assert.match(worker, /\.select\('enabled,per_turn_token_limit,conversation_turn_limit'\)/);
  assert.match(worker, /if \(data\.enabled !== true\) return \{ enabled: false, limits: null \}/);
  assert.match(worker, /const countAiReplies = async/);
  assert.match(worker, /\.select\('id'\)[\s\S]*?\.eq\('sender_type', 'ai'\)[\s\S]*?\.limit\(conversationTurnLimit\)/);
  assert.match(worker, /p_escalation_reason: 'conversation_turn_limit'/);
  assert.match(worker, /max_tokens: perTurnTokenLimit/);
  ...
  const workerActivityQuery = adminApi.match(/const workerActivityResult =([\s\S]*?)if \(workerActivityResult\.error/);
  assert.ok(workerActivityQuery);
  assert.match(workerActivityQuery[1], /\.from\('support_ai_runs'\)/);
  assert.match(workerActivityQuery[1], /\.select\('status,updated_at,completed_at'\)/);
  assert.match(workerActivityQuery[1], /\.order\('updated_at', \{ ascending: false \}\)/);
  assert.match(workerActivityQuery[1], /\.limit\(1\)/);
  assert.doesNotMatch(workerActivityQuery[1], /answer|citations|conversation_id|last_error/i);
  assert.match(adminApi, /workerActivity: workerActivityResult\.error/);
  assert.match(usageMigration, /security invoker/i);
  assert.doesNotMatch(usageMigration, /security definer/i);
  assert.match(usageMigration, /revoke all on function public\.admin_read_support_ai_usage_summary\(\) from public, anon, authenticated/i);
  assert.match(usageMigration, /grant execute on function public\.admin_read_support_ai_usage_summary\(\) to service_role/i);
  assert.match(adminApi, /adminClient\.rpc\('admin_read_support_ai_usage_summary'\)/);
  assert.match(adminApi, /usageSummary: usageSummaryResult\.error/);
  assert.match(emailHealthMigration, /security invoker/i);
  assert.doesNotMatch(emailHealthMigration, /security definer/i);
  assert.match(emailHealthMigration, /revoke all on function public\.admin_read_support_email_delivery_health\(\) from public, anon, authenticated/i);
  assert.match(emailHealthMigration, /grant execute on function public\.admin_read_support_email_delivery_health\(\) to service_role/i);
  assert.match(emailHealthMigration, /Sent means provider API acceptance, not recipient delivery/i);
  assert.match(adminApi, /adminClient\.rpc\('admin_read_support_email_delivery_health'\)/);
  assert.match(adminApi, /supportNotifications:/);
  assert.match(adminApi, /workerSecretPresent: Boolean/);
  assert.match(adminApi, /providerKeyPresent: Boolean/);
  assert.match(adminApi, /senderValuePresent: Boolean/);
  assert.match(adminApi, /deliveryHealth: emailDeliveryHealthResult\.error/);
  assert.match(dashboard, /Support email queue health/);
  assert.match(dashboard, /Configuration indicators report value presence only/);
  assert.match(dashboard, /not confirmation that the recipient received the email/);
  ...
  assert.match(adminApi, /if \(enabled && Deno\.env\.get\('SUPPORT_AI_ENABLED'\) !== 'true'\)/);
  ...
  assert.match(dashboard, /Latest recorded run/);
  assert.match(dashboard, /Usage and cost coverage · last 30 days/);
  assert.match(dashboard, /Provider-reported cost units and currency are unverified/);
  assert.match(dashboard, /do not test live provider connectivity/);
  assert.match(dashboard, /Save AI safety settings/);
  assert.match(dashboard, /Max response tokens/);
  assert.match(dashboard, /Max AI replies per conversation/);
  assert.match(dashboard, /USD budget fields are not an enforced spending limit/);
  assert.match(dashboard, /const canEnableSupportAi = Boolean\(supportAi\?\.runtimeEnabled && supportAi\?\.providerConfigured && supportAi\?\.workerConfigured\)/);
  assert.match(dashboard, /!form\.enabled && !canEnableSupportAi/);
