...
DO $$ BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname='audit_auth_admin') THEN CREATE ROLE audit_auth_admin NOLOGIN; END IF;
END $$;
GRANT anon, authenticated, service_role, audit_auth_admin TO postgres;
CREATE SCHEMA auth;
